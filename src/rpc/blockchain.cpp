...
UniValue loadtxoutset(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "(Phase B2/D). Allowed on regtest, with -assumeutxodev=1, or when height+hash_serialized\n"
            "matches an attested entry in chainparams mapAssumeutxo.\n"
...
UniValue activatesnapshot(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "Wallet and chainActive use the snapshot tip; former IBD tip is parked for Phase C.\n"
            "Allowed: regtest, -assumeutxodev=1, or attested mapAssumeutxo height+hash match.\n"
            "\nResult:\n"
            "{\n"
            "  \"active_swapped\": true,\n"
