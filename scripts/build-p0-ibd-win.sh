#!/usr/bin/env bash
# Rebuild P0 IBD telemetry into Windows dogecoind + dogecoin-qt and package.
set -euo pipefail
...
echo "[1/6] Sync sources from /mnt/c/dogedev"
...
echo "[2/6] Compile server objects (automake paths)"
...
echo "[4/6] Rebuild libdogecoin_server.a (correct object set)"
# Object list mirrors make's libdogecoin_server_a_OBJECTS (subdir-aware).
SERVER_OBJS=(
...
# dogecoin.cpp / dogecoin-fees may live in common, not server — check
...
for o in "${SERVER_OBJS[@]}"; do
  if [[ ! -f "$o" ]]; then
    echo "MISSING object: $o"
    missing=1
  fi
done
if [[ "$missing" -ne 0 ]]; then
  echo "ERROR: cannot rebuild server archive"
  exit 1
fi

rm -f libdogecoin_server.a
# ar stores basenames; put subdir objects under unique temp names to avoid collisions
STAGE=$(mktemp -d)
...
for o in "${SERVER_OBJS[@]}"; do
  base=$(basename "$o")
  # uniquify collisions (net.o from root vs rpc/)
  dest="$STAGE/${i}_$base"
  cp -f "$o" "$dest"
  STAGED+=("$dest")
  i=$((i + 1))
...
echo "  members=$(x86_64-w64-mingw32-ar t libdogecoin_server.a | wc -l)"
x86_64-w64-mingw32-nm libdogecoin_server.a | grep -E 'getibdinfo|NoteStallDisconnect|getpeerinfo' | head -12

echo "[5/6] Update libdogecoinqt.a and link binaries"
...
make dogecoind.exe 2>&1 | tail -25
make qt/dogecoin-qt.exe 2>&1 | tail -30

echo "Verify embedded strings"
x86_64-w64-mingw32-strings dogecoind.exe | grep -E 'getibdinfo|ibdrescue|stall disconnect' | head -15
x86_64-w64-mingw32-strings qt/dogecoin-qt.exe | grep -E 'getibdinfo|IBD telemetry|ibdrescue|Arcade|Shibe' | head -20
file dogecoind.exe qt/dogecoin-qt.exe

echo "[6/6] Package release"
bash /mnt/c/dogedev/scripts/package-windows-release.sh 2>&1 | tail -50
